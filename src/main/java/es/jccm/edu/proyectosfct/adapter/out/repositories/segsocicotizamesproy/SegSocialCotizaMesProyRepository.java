package es.jccm.edu.proyectosfct.adapter.out.repositories.segsocicotizamesproy;
...
import es.jccm.edu.proyectosfct.application.domain.segsocicotizamesproyectos.entities.QCotizaMesProyectos;
import es.jccm.edu.shared.adapter.out.repositories.AbstractRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.stereotype.Repository;

@Repository
public interface SegSocialCotizaMesProyRepository  extends AbstractRepository<CotizaMesProyectos, Long, QCotizaMesProyectos> {

 List<CotizaMesProyectos> findByIdConvProyAlu(Long id);
 ...
 @Query(value = " select distinct alu.T_APELLIDO1 || ' ' || alu.T_APELLIDO2 || ',' || alu.T_NOMBRE as nombreCompleto,   "
   ...
   + "                    FROM fct_historicomes_proy   "
   + "                    WHERE id_cotizames_proy = cot.id_cotizames_proy),2,'0')  as nuPeticion,           "
   + "              CASE    "
   + "                  WHEN hag.id_historicomes_proy is null then 'Alta'    "
   + "                  ELSE 'Modificacion'   "
   + "                            END as accion,   "
   ...
   + "                               ELSE 'Diciembre '  "
   + "                               END as mes "
   + "        from fct_cotizames_proy cot, tlmatalu mat, tlalumnos alu, fct_historicomes_proy hag   "
   + "        where mat.x_matricula = cot.x_matricula   "
   + "        and alu.x_alumno = mat.x_alumno   "
   + "        and cot.id_cotizames_proy = hag.id_cotizames_proy(+)   "
   + "        and cot.id_cotizames_proy = :idCotizaMes   "
   + "        union all   "
   ...
   + "                       hag.NU_DIAS_NACU as nuDiasNacuMesPorg,  "
   ...
   + "                                    ELSE 'Diciembre '  "
   + "                               END as mes  "
   + "        from fct_historicomes_proy hag,   "
   + "            ( SELECT alu.T_APELLIDO1 || ' ' || alu.T_APELLIDO2 || ',' || alu.T_NOMBRE as nombre   "
   + "              FROM fct_cotizames_proy cot, tlmatalu mat, tlalumnos alu    "
   + "              WHERE mat.x_matricula = cot.x_matricula   "
   + "              AND alu.x_alumno = mat.x_alumno   "
   + "              AND cot.id_cotizames_proy = :idCotizaMes   "
   + "            ) cot   "
   + "        where hag.id_cotizames_proy =  :idCotizaMes   "
 ...
 List<ListadoHistoricoCotizaMesProjection> getListadoHistoricoMes(Long idCotizaMes);
    ...
    @Query(value = "select count(*) " +
            "              from fct_cotizames_proy " +
            "              where id_cotizames_proy = :id " +
    ...
    CotizaMesProyectos findByIdConvProyAluAndNuMes(Long idConvProg, Integer nuMes);

    @Query(value = "select * from fct_cotizames_proy where x_matricula = :xMatricula and nu_mes = :nuMes", nativeQuery = true)
    CotizaMesProyectos findByXmatriculaAndNuMes(Long xMatricula, Integer nuMes);

    @Query(value = "select tlf_parametro('MAXDIA_LIQUIDACION_MENSUAL') from dual" , nativeQuery = true)
    Integer getMaxDiaLiquidacionMensual();

    @Query(value = "SELECT c.* " +
            "FROM FCT_COTIZAMES_PROY c " +
            "JOIN FCT_ALTASS_PROY a ON c.ID_CONVPROY_ALU = a.ID_CONVPROY_ALU AND c.X_MATRICULA = a.X_MATRICULA " +
            "WHERE c.F_ENVIOSS IS NULL " +
            "AND a.ID_ALTASS_PROY = :idAltaProy " +
            "AND TO_DATE('01-' || LPAD(c.NU_MES, 2, '0') || '-' || " +
            "           CASE " +
            //"               WHEN c.NU_MES IN (1,2,3,4,5,6,7,8) THEN TO_CHAR(a.F_INICIO + INTERVAL '1' YEAR) " +
            "               WHEN c.NU_MES IN (9,10,11,12) THEN TO_CHAR(a.F_INICIO + INTERVAL '1' YEAR) " +
            "               ELSE TO_CHAR(a.F_INICIO, 'YYYY') " + 
            "           END, 'DD-MM-YYYY') " +
            " BETWEEN TRUNC(a.F_INICIO, 'MM') AND LAST_DAY(a.F_FIN) " +   
            " AND not exists ( SELECT 1       " +
            "                  FROM FCT_ALTASS_PROY aly2     " +   
            "                  WHERE aly2.X_MATRICULA = a.X_MATRICULA " +       
            "                  AND aly2.ID_ALTASS_PROY <> a.ID_ALTASS_PROY " +       
    ...
    List<CotizaMesProyectos> findCotizaMesNoEnviadaSolapadaAltaProy(Long idAltaProy);
